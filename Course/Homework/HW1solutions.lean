...
example (a b : ℝ): a^4 - b^4 = (a^2 - b^2) * (a^2 + b^2) := by
  rw [mul_add, sub_mul, sub_mul, add_sub, ← pow_add, ← pow_add, mul_comm, sub_add, sub_self, sub_zero]
  -- rw [sub_mul, mul_add, mul_add, mul_comm (a^2) (b^2), ← sub_sub, ← add_sub, sub_self, add_zero, ← pow_add, ← pow_add]
...
example (a b c d : ℝ) (h : a ≤ 4 * c + b) (h' : c ≤ 2 * d) (h'' : b = 3 * d) : a ≤ 11 * d := by
  calc
    _ ≤ 4 * c + b := h
    _ = 4 * c + 3 * d := by rw [h'']
    _ ≤ 8 * d + 3 * d := add_le_add (le_of_le_of_eq (mul_le_mul_of_nonneg_left h' (by norm_num)) (by ring)) (by rfl)
