...
theorem riemannLebesgue {a b : ℝ} (hab : a ≤ b) (hf : ∀ x ∈ uIcc a b, HasDerivAt f (f' x) x)
  ...
  have exists_forall_abs_integral_sin_mul_le : ∃ C, ∀ R > 0, |∫ x in a..b, sin (R * x) * f x| ≤ C * R⁻¹ := by
    ...
    calc
      ...
      _ ≤ R⁻¹ * (|cos (R * b) * f b - cos (R * a) * f a| + |∫ (x : ℝ) in a..b, cos (R * x) * f' x|) := by sorry
      _ ≤ R⁻¹ * (|cos (R * b)| * |f b| + |cos (R * a)| * |f a| + |∫ (x : ℝ) in a..b, cos (R * x) * f' x|) := by sorry
      _ ≤ R⁻¹ * (1 * |f b| + 1 * |f a| + ∫ x in a..b, |cos (R * x) * f' x|) := by sorry
  ...
  have C_nonneg : 0 ≤ C := by -- Using that the absolute value is always non-negative
    calc
      0 ≤ |∫ (x : ℝ) in a..b, sin (1 * x) * f x| := sorry
      _ ≤ C * 1⁻¹ := sorry
      _ = _ := by simp
  by_cases C_zero : C = 0
  · -- If `C = 0` the claim follows using `hC` and that `0 < ε`
    use 1
    sorry
  · use 2 * C * ε⁻¹
    intro R hR
